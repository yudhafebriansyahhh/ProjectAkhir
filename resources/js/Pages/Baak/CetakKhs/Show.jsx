...
export default function Show({ krs, rows = [], summary = {} }) {
    return (
        <>
            <Head title={`Cetak KHS ${krs?.mahasiswa?.nama || ''}`} />
            <style>{`
                ...
                .print-field { display: grid; grid-template-columns: 30mm 3mm 1fr; line-height: 1.45; }
            ...
            <div className="print-shell">
                <div className="print-toolbar no-print">
                    <Link href={route('baak.cetak-khs.index')}><Button type="button" variant="outline" className="gap-2"><ArrowLeft className="h-4 w-4" />Kembali</Button></Link>
                    <Button type="button" className="gap-2" onClick={() => window.print()}><Printer className="h-4 w-4" />Cetak KHS</Button>
                </div>
                <main className="print-page">
                    ...
                    <h1 className="print-title">Kartu Hasil Studi (KHS)</h1>
                    <section className="identity-grid">
                        ...
                        <div>
                            ...
                            <Field label="Dosen Wali" value={krs?.mahasiswa?.dosen_wali} />
                        </div>
                    </section>
                    <table className="print-table">
                        <thead>
                            <tr>
                                ...
                                <th rowSpan="2" style={{ width: '24mm' }}>KODE MK</th>
                                <th rowSpan="2">NAMA MATA KULIAH</th>
                                <th rowSpan="2" style={{ width: '10mm' }}>SKS</th>
                                <th colSpan="2">NILAI</th>
                                <th rowSpan="2" style={{ width: '14mm' }}>BOBOT</th>
                                <th rowSpan="2" style={{ width: '16mm' }}>B X S</th>
                            </tr>
                            <tr>
                                <th style={{ width: '16mm' }}>ANGKA</th>
                                <th style={{ width: '14mm' }}>HURUF</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            )) : <tr><td colSpan="8" className="center">Belum ada nilai pada semester ini.</td></tr>}
                            <tr>
                                <td colSpan="3" className="center"><strong>JUMLAH</strong></td>
                                <td className="center"><strong>{summary.total_sks || 0}</strong></td>
                                <td colSpan="3" />
                                <td className="right"><strong>{formatNumber(summary.total_bxs)}</strong></td>
                            </tr>
                        </tbody>
                    </table>
                    <section className="summary">
                        <div><Field label="Jumlah SKS Semester" value={summary.total_sks || 0} /></div>
                        <div><Field label="Indeks Prestasi Semester" value={summary.ips || '0,00'} /></div>
                    </section>
