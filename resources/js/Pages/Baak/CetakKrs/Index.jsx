...
import { CheckCircle2, Clock3, Eye, FileText, Printer, RefreshCcw } from 'lucide-react';
...
export default function Index({ pengajuans, prodis = [], tahunAjaranList = [], stats = {}, filters = {} }) {
    ...
    const resetFilters = () => {
        ...
        setSemester('');
        setStatusPengajuan('');
        router.get(route('baak.cetak-krs.index'), {}, { preserveState: true, preserveScroll: true, replace: true });
    };
    ...
    const renderAction = (item, compact = false) => (
        <div className={`flex items-center justify-center gap-2 ${compact ? 'w-full' : ''}`}>
            <Link href={route('baak.cetak-krs.show', item.id_krs)} className={compact ? 'flex-1' : ''}>
    ...
    return (
        <BaakLayout title="Ajuan Cetak KRS">
            <Head title="Ajuan Cetak KRS" />
            <div className="min-h-screen bg-slate-50 px-3 py-4 sm:px-4 sm:py-5 md:px-6 lg:px-8">
                <div className="mx-auto w-full max-w-[1440px] space-y-4 md:space-y-5">
                    <PageHeader title="Ajuan Cetak KRS" description="Kelola pengajuan cetak kartu rencana studi dari mahasiswa." icon={Printer} />
                    ...
                    <DataTable columns={columns} data={pengajuanData} getRowKey={(item) => item.id_pengajuan} emptyState={<EmptyState title="Tidak ada Ajuan" description="Data ajuan cetak KRS tidak ditemukan atau belum sesuai filter." />} className="hidden lg:block" />
                    <CardGrid data={pengajuanData} getCardKey={(item) => item.id_pengajuan} renderCard={renderCard} emptyState={<EmptyState title="Tidak ada Ajuan" description="Data ajuan cetak KRS tidak ditemukan atau belum sesuai filter." compact />} className="grid gap-3 md:grid-cols-2 lg:hidden" emptyClassName="lg:hidden" />
